...
import * as Autocomplete from "@danielfrg/ui/autocomplete";
...
export function DemoAutocompleteHero() {
	...
	const onInputChange = (value: string) => {
		const filtered = ALL_FRUITS.filter((fruit) =>
			fruit.label.toLowerCase().includes(value.toLowerCase()),
		);
		setOptions(filtered);
	};
		...
		>
			...
			<Autocomplete.Portal>
				<Autocomplete.Content class={styles.content}>
					...
					<Autocomplete.NoResult class={styles["no-result"]}>
						No results found.
