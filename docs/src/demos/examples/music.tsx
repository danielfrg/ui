...
function PlayIcon(props: { size?: number }) {
	...
	return <svg width={s} height={s} viewBox="0 0 30 30" fill="currentColor"><path d="M 6 3 A 1 1 0 0 0 5 4 L 5 26 A 1 1 0 0 0 6.58 26.81 L 26.42 15.91 A 1 1 0 0 0 27 15 A 1 1 0 0 0 26.39 14.08 L 6.58 3.19 A 1 1 0 0 0 6 3 z" /></svg>
}
...
function CheckIcon() {
	return <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor"><path d="M9.16 1.12C9.51 1.35 9.6 1.81 9.38 2.16L5.14 8.66C5.02 8.84 4.82 8.97 4.6 8.99C4.39 9.02 4.17 8.95 4.01 8.81L1.25 6.31C0.94 6.03 0.92 5.56 1.19 5.25C1.47 4.94 1.95 4.92 2.25 5.2L4.36 7.1L8.12 1.34C8.35 0.99 8.81 0.9 9.16 1.12Z" /></svg>
}
...
const songsQueue = [
	{ name: "A Certain Romance", artist: "Arctic Monkeys", album: "Whatever People Say I Am", cover: coverSmall, length: "5:31" },
	...
	{ name: "Fluorescent Adolescent", artist: "Arctic Monkeys", album: "Favourite Worst Nightmare", cover: coverSmall, length: "2:58" },
]
...
const songsHistory = [
	...
	{ name: "Under the Bridge", artist: "Red Hot Chili Peppers", album: "Blood Sugar Sex Magik", cover: coverSmall, length: "4:24" },
		...
		<div class={styles.albumCard}>
			...
			/>
				<div class={styles.albumPlay}>
					<button class={styles.iconBtnPlay}><PlayIcon size={16} /></button>
				</div>
			</div>
	...
	return (
		<div class={styles.songRow}>
			...
			<div class={styles.songInfo}>
				...
				<div class={`${styles.textXs} ${styles.textMuted} ${styles.textTruncate}`}>{props.song.artist} – {props.song.album}</div>
			</div>
		...
		<div class={styles.layout}>
			...
			<div class={`${styles.col} ${styles.colSidebar}`}>
				...
				<div class={styles.card}>
					...
					<div class={`${styles.flexCenter} ${styles.gap2} ${styles.mb5}`} style={{ height: "16px", "margin-top": "8px" }}>
											<VolumeNoneIcon />
				...
				<div class={styles.card}>
					...
					<div style={{ position: "relative" }}>
						...
						<div class={styles.eqGrid}>
							{[
								{ f: "32", v: 75 }, { f: "64", v: 73 }, { f: "125", v: 70 }, { f: "250", v: 65 },
								{ f: "500", v: 52 }, { f: "1K", v: 50 }, { f: "2K", v: 56 }, { f: "3K", v: 57 },
								{ f: "4K", v: 60 }, { f: "6K", v: 62 }, { f: "8K", v: 65 }, { f: "16K", v: 68 },
							].map((s) => (
								<div class={styles.eqSlider}>
									<input type="range" class={`${styles.slider} ${styles.sliderSm} ${styles.sliderGray} ${styles.sliderVertical}`} value={s.v} min="0" max="100" style={{ height: "120px" }} />
			...
			<div class={`${styles.col} ${styles.colMain}`}>
				...
				<div class={`${styles.card} ${styles.cardLg}`} style={{ height: "896px" }}>
					...
					<div class={styles.toolbar}>
						<button class={`${styles.iconBtn} ${styles.iconBtnRound}`}><HeartIcon /></button>
						<button class={`${styles.iconBtn} ${styles.iconBtnRound}`}><CounterClockwiseClockIcon /></button>
						<button class={`${styles.iconBtn} ${styles.iconBtnRound}`}><MixerHorizontalIcon /></button>
					...
					<div class={`${styles.flexBetween} ${styles.mb4}`}>
						...
						<a class={`${styles.link} ${styles.textSm} ${styles.textMedium}`} href="#" onClick={(e) => e.preventDefault()}>Show all</a>
					</div>
					<div class={`${styles.grid5} ${styles.gap5} ${styles.mb7}`}>
						<For each={albumsFavorites}>{(a) => <AlbumCard title={a.name} caption={a.artist} cover={a.cover} color={a.color} />}</For>
					</div>
					...
					<div class={`${styles.grid5} ${styles.gap5} ${styles.mb7}`}>
						<For each={playlistsForYou}>{(p) => <AlbumCard title={p.title} caption={p.caption} cover={p.cover} color={p.color} />}</For>
					</div>
					...
					<div class={styles.nowPlaying}>
						...
						<div class={styles.nowPlayingContent}>
							<div class={`${styles.flexCenter} ${styles.gap4}`} style={{ padding: "12px" }}>
								...
								<button class={`${styles.iconBtn} ${styles.iconBtnRound}`}>
									<svg width="16" height="16" viewBox="0 0 30 30" fill="currentColor"><path d="M 21 5 L 21 8 L 18.675781 8 C 16.670448 8 14.796256 9.00408 13.683594 10.671875 L 12 13.197266 L 10.316406 10.671875 C 9.2045791 9.0047337 7.329552 8 5.3242188 8 L 3 8 L 3 10 L 5.3242188 10 C 6.6628853 10 7.910171 10.668391 8.6523438 11.78125 L 10.798828 15 L 8.6523438 18.21875 C 7.910171 19.331609 6.6628854 20 5.3242188 20 L 3 20 L 3 22 L 5.3242188 22 C 7.3295521 22 9.2045792 20.995266 10.316406 19.328125 L 12 16.802734 L 13.683594 19.328125 C 14.796256 20.99592 16.670448 22 18.675781 22 L 21 22 L 21 25 L 27 21 L 21 17 L 21 20 L 18.675781 20 C 17.337115 20 16.090994 19.332955 15.347656 18.21875 L 13.201172 15 L 15.347656 11.78125 C 16.090994 10.667045 17.337115 10 18.675781 10 L 21 10 L 21 13 L 27 9 L 21 5 z" /></svg>
								</button>
							</div>
							<div class={`${styles.flexCenter} ${styles.gap3}`}>
								<button class={`${styles.iconBtn} ${styles.iconBtnRound}`}>
									<svg width="20" height="20" viewBox="0 0 30 30" fill="currentColor" style={{ opacity: 0.7 }}><path d="M 14 6 L 2.46 14.16 L 2.44 14.17 A 1 1 0 0 0 2 15 A 1 1 0 0 0 2.45 15.83 L 13.41 23.8 A 1 1 0 0 0 15 23 L 15 16.23 L 25.41 23.8 A 1 1 0 0 0 27 23 L 27 7 A 1 1 0 0 0 25.41 6.2 L 15 13.76 L 15 7 A 1 1 0 0 0 13.41 6.2 Z" /></svg>
								</button>
								<div class={`${styles.flexCenter} ${styles.gap3}`}>
									...
									<div>
										...
										<div class={`${styles.textXs} ${styles.textMuted} ${styles.mb2}`}>{songsHistory[6]?.artist} – {songsHistory[6]?.album}</div>
								...
								<button class={`${styles.iconBtn} ${styles.iconBtnRound}`}>
									<svg width="20" height="20" viewBox="0 0 30 30" fill="currentColor" style={{ opacity: 0.7 }}><path d="M 4 6 A 1 1 0 0 0 3 7 L 3 23 A 1 1 0 0 0 4.59 23.8 L 15 16.24 L 15 23 A 1 1 0 0 0 16.59 23.8 L 27.54 15.84 A 1 1 0 0 0 28 15 A 1 1 0 0 0 27.56 14.17 L 16.59 6.2 A 1 1 0 0 0 15 7 L 15 13.77 L 4.59 6.2 A 1 1 0 0 0 4 6 z" /></svg>
								</button>
							</div>
							<div class={`${styles.flexCenter} ${styles.gap2}`} style={{ padding: "20px" }}>
								<VolumeNoneIcon />
								<input type="range" class={`${styles.slider} ${styles.sliderSm} ${styles.sliderGray}`} value="80" style={{ width: "80px" }} />
								<VolumeMaxIcon />
							</div>
						</div>
					</div>
					...
					<div class={`${styles.col} ${styles.colSub}`}>
						<div class={styles.card}>
							...
							<div class={`${styles.flexCol} ${styles.gap5}`}>
								{[
									{ label: "Automatic downloads", desc: "Automatically download music when added to your library", on: false },
									{ label: "Lossless audio", desc: "Preserve every detail of the original audio, but consume significantly more data", on: false },
								...
								<div>
									<div class={styles.settingsRow}><div class={styles.settingsInfo}><span class={`${styles.textSm} ${styles.textMedium} ${styles.mb1}`}>Maximum volume</span><span class={`${styles.textXs} ${styles.textMuted}`}>Limit the maximum volume to protect hearing</span></div></div>
									<div class={`${styles.flexCenter} ${styles.gap2} ${styles.mt2}`}>
										<span class={`${styles.textXs} ${styles.textMuted}`} style={{ width: "24px" }}>0%</span>
										<input type="range" class={`${styles.slider} ${styles.sliderSm} ${styles.flexGrow}`} value="80" />
										<span class={`${styles.textXs} ${styles.textMuted}`} style={{ width: "32px" }}>100%</span>
									</div>
								</div>
								<div>
									<div class={styles.settingsRow}><div class={styles.settingsInfo}><span class={`${styles.textSm} ${styles.textMedium} ${styles.mb1}`}>Crossfade</span><span class={`${styles.textXs} ${styles.textMuted}`}>Smoothly fade out into the next song.</span></div></div>
									<div class={`${styles.flexCenter} ${styles.gap2} ${styles.mt2}`}>
										<span class={`${styles.textXs} ${styles.textMuted}`} style={{ width: "24px" }}>Off</span>
										<input type="range" class={`${styles.slider} ${styles.sliderSm} ${styles.flexGrow}`} value="0" />
										<span class={`${styles.textXs} ${styles.textMuted}`} style={{ width: "32px" }}>10s</span>
									</div>
								</div>
							</div>
						</div>
							...
							<div class={`${styles.flexCol} ${styles.gap4}`}>
								{[
									...
									<div class={styles.memberRow}>
										...
										<Button class={`${styles.btn} ${tier.active ? styles.btnPrimary : styles.btnSoft}`} style={{ width: "64px" }}>{tier.price}</Button>
									</div>
								))}
							</div>
							<Separator class={styles.separator} style={{ margin: "20px 0" }} />
							<p class={`${styles.textSm}`} style={{ margin: "12px 0" }}>Your next payment is $6.99 on {new Date(Date.now() + 20 * 86400000).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
							<a class={`${styles.link} ${styles.textSm}`} href="#" onClick={(e) => e.preventDefault()}>Cancel subscription</a>
						</div>
					</div>
					...
					<div class={`${styles.col} ${styles.colSub}`}>
						<div class={styles.card}>
							...
							<div class={`${styles.flexCol} ${styles.gap4}`}>
									...
									{[
										...
										<Checkbox.Root defaultChecked={item.checked} class={`${styles.flexCenter} ${styles.gap2}`}>
											...
											<Checkbox.Control class={styles.checkboxControl}>
												<Checkbox.Indicator class={styles.checkboxIndicator}><CheckIcon /></Checkbox.Indicator>
											</Checkbox.Control>
								...
								<div class={`${styles.flexCol} ${styles.gap2}`}>
									...
									<a class={`${styles.link} ${styles.textSm}`} href="#" onClick={(e) => e.preventDefault()}>Reset recommendations</a>
									<a class={`${styles.link} ${styles.textSm}`} href="#" onClick={(e) => e.preventDefault()}>Delete profile</a>
								</div>
							</div>
						</div>
						...
						<div class={styles.card}>
							...
							<div class={`${styles.flex} ${styles.gap2} ${styles.flexWrap}`}>
								{["Pop","Rock","Hip-Hop","R&B","Country","Jazz","Blues","Electronic","Experimental","Reggae","Ska","Grunge","Psychedelic","Post-Rock","Progressive Rock","Dream Pop","Ambient","Classical","Trip-Hop","Gospel","Folk","Indie","Alternative","Punk","Metal","Funk","Soul","Dance","Techno","Acoustic","House","Dubstep","Latin","Salsa","K-pop"].map((genre) => (
									<Button class={`${styles.btn} ${["Blues","Jazz","Funk"].includes(genre) ? styles.btnPillActive : `${styles.btnSoft} ${styles.btnPill}`}`}>
										{genre}
									</Button>
								))}
							</div>
						...
						/>
							...
							<div class={styles.mt5}>
								<div class={styles.stars}>
									{[1,2,3,4].map(() => (
										<svg width="20" height="20" viewBox="0 0 30 30" fill="currentColor"><path d="M15.765,2.434l2.875,8.512l8.983,0.104c0.773,0.009,1.093,0.994,0.473,1.455l-7.207,5.364l2.677,8.576 c0.23,0.738-0.607,1.346-1.238,0.899L15,22.147l-7.329,5.196c-0.63,0.447-1.468-0.162-1.238-0.899l2.677-8.576l-7.207-5.364 c-0.62-0.461-0.3-1.446,0.473-1.455l8.983-0.104l2.875-8.512C14.482,1.701,15.518,1.701,15.765,2.434z" /></svg>
									))}
									<svg width="20" height="20" viewBox="0 0 30 30" fill="currentColor"><path d="M15 3.6l-3.5 10.7H.5l9 6.5-3.4 10.6 9-6.5 9 6.5-3.4-10.6 9-6.5H19.5z" opacity="0.3" /></svg>
								</div>
								<h3 class={`${styles.h3} ${styles.textCenter} ${styles.mb2}`}>King Krule – The OOZ</h3>
								<p class={`${styles.textSm} ${styles.textMuted} ${styles.textCenter} ${styles.mb4}`}>A dark and introspective album that showcases King Krule's distinctive blend of genres, while delivering hauntingly raw and poetic lyrics.</p>
								<div class={`${styles.flex} ${styles.justifyCenter} ${styles.gap3}`}>
									<Button class={`${styles.btn} ${styles.btnPrimary}`}>Listen Now</Button>
									<button class={`${styles.iconBtn} ${styles.iconBtnSoftGray}`} style={{ "border-radius": "8px" }}><HeartIcon /></button>
								</div>
							</div>
						</div>
