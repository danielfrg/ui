...
function Marker(props: { children: JSX.Element; large?: boolean }) {
	return (
		<span class={`${styles.marker} ${props.large ? styles.markerLg : ""}`}>
			{props.children}
		</span>
	)
}
...
const todoContent: Record<string, () => JSX.Element> = {
	a: () => (
		<span>
			Respond to comment{" "}
			<a class={styles.link} href="#" onClick={(e) => e.preventDefault()}>#384</a>{" "}
			from Travis Ross
		</span>
	),
	b: () => (
		<span>
			Invite{" "}
			<a class={styles.link} href="#" onClick={(e) => e.preventDefault()}>Acme Co.</a>{" "}
			team to Slack
		</span>
	),
	c: () => (
		<span>
			Create a report{" "}
			<a class={styles.link} href="#" onClick={(e) => e.preventDefault()}>requested</a>{" "}
			by Danilo Sousa
		</span>
	),
	d: () => (
		<span>
			...
			<a class={styles.link} href="#" onClick={(e) => e.preventDefault()}>#85</a>
		</span>
	),
	...
	f: () => (
		<span>
			Review invoice{" "}
			<a class={styles.link} href="#" onClick={(e) => e.preventDefault()}>#3456</a>
		</span>
	),
}
		...
		<div class={`${styles.flexCol} ${styles.gap2}`}>
			<For each={props.items}>
				{(item) => (
					...
					<span
						class={item.completed ? styles.todoCompleted : ""}
		...
		<div class={styles.layout}>
			...
			<div class={`${styles.col} ${styles.col1}`}>
									...
									<div class={styles.teamRow}>
										<div class={styles.teamInfo}>
											<Avatar.Root class={styles.avatarRoot}>
												<Avatar.Image class={styles.avatarImage} src={allPeople[number]?.image} alt={allPeople[number]?.name} />
												<Avatar.Fallback class={styles.avatarFallback}>{allPeople[number]?.name[0].toUpperCase()}</Avatar.Fallback>
											</Avatar.Root>
											<a class={`${styles.link} ${styles.linkSubtle} ${styles.teamName}`} href="#" onClick={(e) => e.preventDefault()}>
				...
				<div class={styles.card}>
					...
					{[
						{ title: "Comments", desc: "Receive notifications when someone comments on your documents or mentions you.", push: true, emailOn: true, slack: false },
						{ title: "Favorites", desc: "Receive notifications when there is activity related to your favorited items.", push: true, emailOn: true, slack: false },
						{ title: "New documents", desc: "Receive notifications whenever people on your team create new documents.", push: true, emailOn: true, slack: false },
					].map((notif, idx) => (
						<>
							<div class={`${styles.notifRow} ${styles.my5}`}>
								<div>
									<h4 class={styles.h4}>{notif.title}</h4>
									<p class={`${styles.textSm} ${styles.textMuted}`} style={{ "margin-top": "4px" }}>{notif.desc}</p>
								</div>
				...
				<div class={styles.card}>
					...
					<p class={`${styles.subtitle} ${styles.mb5}`}>No credit card required. Every plan includes a 30-day trial of all Pro features.</p>
					<div class={styles.grid3}>
						{[
							{ name: "Basic", members: "3 team members", price: "$0", features: ["Expense tracking", "Invoicing", "Payment tracking", "Transaction recording", "Basic reports", "Email support"], action: "Downgrade", variant: "outline" as const },
							{ name: "Growth", members: "10 team members", price: "$49", features: ["Online payments", "Recurring invoices", "Bill management", "Inventory tracking", "Detailed reports", "Phone support"], action: "Go to Billing", variant: "outline" as const },
							{ name: "Pro", members: "Unlimited team members", price: "$99", features: ["Custom invoices", "Multi-business", "Team collaboration", "App integrations", "Advanced security", "Priority support"], action: "Upgrade", variant: "primary" as const },
						].map((tier) => (
							<div class={styles.flexCol}>
								...
								<span class={`${styles.textLg} ${styles.textBold} ${styles.mb4}`}>
									{tier.price}<span class={styles.priceMuted}>{" / mo"}</span>
								</span>
								<div class={`${styles.flexCol} ${styles.gap2}`}>
									{tier.features.map((f) => (
										<div class={`${styles.flexCenter} ${styles.gap2}`}>
											<Marker><CheckIcon width={10} height={10} /></Marker>
			...
			<div class={`${styles.col} ${styles.col2}`}>
				...
				<div class={styles.card}>
					...
					<div class={styles.mb5}>
						<label class={`${styles.textSm} ${styles.textBold}`} style={{ display: "block", "margin-bottom": "4px" }}>Email address</label>
						<input class={styles.inputField} style={{ width: "100%", "box-sizing": "border-box" }} placeholder="Enter your email" />
					</div>
					<div class={styles.mb5}>
						<div class={styles.flexBetween} style={{ "margin-bottom": "4px" }}>
							<label class={`${styles.textSm} ${styles.textBold}`}>Password</label>
							<a class={styles.link} href="#" onClick={(e) => e.preventDefault()}>Forgot password?</a>
						</div>
						<input class={styles.inputField} style={{ width: "100%", "box-sizing": "border-box" }} type="password" placeholder="Enter your password" />
					</div>
				...
				{/* Your Company Card */}
				<div class={styles.card}>
					<button class={`${styles.iconBtn} ${styles.closeBtn}`}><Cross2Icon /></button>
					...
					<div class={styles.creditCardWrap}>
						<div class={styles.creditCard}>
							<span class={styles.textSm} style={{ "font-weight": "500" }}>Sophie Johnson</span>
							<div>
								<div class={`${styles.flexCenter} ${styles.gap3} ${styles.mb1}`}>
									<span class={styles.textSm}>4929 3849 5027 1846</span>
									<button class={styles.iconBtn} style={{ width: "24px", height: "24px", color: "white" }}><CopyIcon /></button>
								</div>
				...
				<div class={styles.card}>
					...
					<div class={`${styles.flexCol} ${styles.gap3}`} style={{ "align-items": "center" }}>
						<Marker large><CheckIcon width={32} height={32} /></Marker>
				...
				<div class={styles.card}>
					...
					<h3 class={`${styles.h3} ${styles.mb5}`}>
						Invoice{" "}
						<a class={styles.link} href="#" style={{ "font-weight": "700" }} onClick={(e) => e.preventDefault()}>#3463</a>
					</h3>
			...
			<div class={`${styles.col} ${styles.col3}`}>
				...
				<div class={styles.card}>
					...
					<div class={styles.cardActions}>
						<button class={styles.iconBtn}><OpenInNewWindowIcon /></button>
						<button
							...
							<div>
								<div class={`${styles.flexCenter} ${styles.gap2} ${styles.mb2}`}>
									...
									<span class={`${styles.badge} ${
										kpi.badge === "teal" ? styles.badgeTeal :
										kpi.badge === "red" ? styles.badgeRed : styles.badgeGray
									}`}>
				...
				<div class={styles.card}>
					...
					<p class={`${styles.subtitle} ${styles.mb6}`} style={{ "margin-bottom": "28px" }}>Review what has happened over the past days.</p>
					{([
						{ person: 6, text: () => <>Approved invoice <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>#3461</a></>, time: "June 21, 11:34 am" },
						{ person: 8, text: () => <>Purchased <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>15 office chairs</a> and <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>2 drum sets</a></>, time: "June 21, 9:43 am" },
						{ person: 8, text: () => <>Responded to your comment <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>#7514</a></>, time: "June 21, 9:41 am" },
						{ person: 28, text: () => <>Created <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>4 invoices</a></>, time: "June 20, 4:55 pm" },
						{ person: 26, text: () => <>Updated client details for <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>Acme Co.</a></>, time: "June 20, 3:30 pm" },
						{ person: 25, text: () => <>Created <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>a new report</a></>, time: "June 20, 3:22 pm" },
						{ person: 25, text: () => <>Deleted report <a class={styles.link} href="#" onClick={(e: Event) => e.preventDefault()}>#34</a></>, time: "June 20, 1:00 pm" },
						{ person: 20, text: () => <>Joined the team</>, time: "June 20, 12:47 pm" },
					] as const).map((act, idx) => (
						<>
							<div class={styles.activityRow} style={{ padding: idx === 0 ? "0 0 20px" : idx === 7 ? "20px 0 0" : "20px 0" }}>
								<div class={styles.activityMeta}>
									<Avatar.Root class={styles.avatarRoot}>
										<Avatar.Image class={styles.avatarImage} src={allPeople[act.person]?.image} alt={allPeople[act.person]?.name} />
										<Avatar.Fallback class={styles.avatarFallback}>{allPeople[act.person]?.name[0].toUpperCase()}</Avatar.Fallback>
									</Avatar.Root>
									<div>
				...
				<div class={styles.card}>
					...
					<div class={styles.cardActions} style={{ gap: "4px" }}>
						<button class={styles.iconBtn}><ShareIcon /></button>
						<button class={styles.iconBtn}><PlusIcon /></button>
					</div>
					...
					<ToDoList
						...
						onToggle={(id, checked) => {
							setTodo((items) =>
								items.map((item) => (item.id === id ? { ...item, completed: checked } : item))
							)
						}}
					/>
				</div>
