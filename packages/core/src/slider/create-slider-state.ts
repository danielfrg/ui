...
import { clamp, mergeDefaultProps, snapValueToStep } from "../utils"
...
export function createSliderState(props: StateOpts): SliderState {
  ...
  const getThumbMinValue = (index: number) => {
    return index === 0
      ...
      : values()[index - 1]! + mergedProps.minStepsBetweenThumbs!() * mergedProps.step!()
  }
  ...
  const getRoundedValue = (value: number) => {
    return (
      Math.round((value - mergedProps.minValue!()) / mergedProps.step!()) * mergedProps.step!() +
      mergedProps.minValue!()
    )
  }
  ...
  const snapThumbValue = (index: number, value: number) => {
    ...
    if (hasMinStepsBetweenValues(nextValues, mergedProps.minStepsBetweenThumbs!() * mergedProps.step!())) {
      updateValue(
        index,
        snapValueToStep(nextValue, mergedProps.minValue!(), mergedProps.maxValue!(), mergedProps.step!()),
      )
    }
  }
