...
export interface ListboxItemIndicatorCommonProps<T extends HTMLElement = HTMLElement> {
  id: string
}

export interface ListboxItemIndicatorRenderProps extends ListboxItemIndicatorCommonProps, ListboxItemDataSet {
  "aria-hidden": "true"
}

export type ListboxItemIndicatorProps<T extends ValidComponent | HTMLElement = HTMLElement> =
  ListboxItemIndicatorOptions & Partial<ListboxItemIndicatorCommonProps<ElementOf<T>>>
...
export function ListboxItemIndicator<T extends ValidComponent = "div">(
  ...
  return (
    <Show when={local.forceMount || context.isSelected()}>
      <Polymorphic<ListboxItemIndicatorRenderProps> as="div" aria-hidden="true" {...context.dataset()} {...others} />
    </Show>
  )
}
