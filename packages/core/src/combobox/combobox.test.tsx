...
describe.skip("Combobox", () => {
  ...
  it("renders correctly", () => {
    const { getByRole, getByText } = render(() => (
      <Combobox.Root
        ...
        itemComponent={(props) => <Combobox.Item item={props.item}>{props.item.rawValue.label}</Combobox.Item>}
      >
  ...
  describe("option mapping", () => {
    ...
    it("supports string based option mapping for object options with string keys", async () => {
      const { getByRole } = render(() => (
        <Combobox.Root<any, any>
          ...
          itemComponent={(props) => <Combobox.Item item={props.item}>{props.item.rawValue.name}</Combobox.Item>}
          sectionComponent={(props) => <Combobox.Section>{props.section.rawValue.name}</Combobox.Section>}
        >
          <Combobox.HiddenSelect />
          <Combobox.Label>Label</Combobox.Label>
          <Combobox.Control>
      ...
      expect(onValueChange.mock.calls[0][0]).toStrictEqual(CUSTOM_DATA_SOURCE_WITH_STRING_KEY[0].items[2])
    ...
    it("supports string based option mapping for object options with number keys", async () => {
      ...
      expect(onValueChange).toHaveBeenCalledTimes(1)
      expect(onValueChange.mock.calls[0][0]).toStrictEqual(CUSTOM_DATA_SOURCE_WITH_NUMBER_KEY[0].items[2])
    ...
    it("supports string options without mapping", async () => {
      const { getByRole } = render(() => (
        <Combobox.Root
          ...
          placeholder="Placeholder"
          onChange={onValueChange}
          itemComponent={(props) => <Combobox.Item item={props.item}>{props.item.rawValue}</Combobox.Item>}
        >
          <Combobox.HiddenSelect />
          <Combobox.Label>Label</Combobox.Label>
          <Combobox.Control>
  ...
  describe("closing", () => {
    ...
    it("can be closed by clicking outside", async () => {
      ...
      fireEvent(document.body, createPointerEvent("pointerup", { pointerId: 1, pointerType: "mouse" }))
      await Promise.resolve()
  ...
  describe("multi-select", () => {
    it("supports selecting multiple options", async () => {
      ...
      expect(
        onValueChange.mock.calls[0][0].map((option: DataSourceItem) => option.key).includes(DATA_SOURCE[0].key),
      ).toBeTruthy()
      expect(
        onValueChange.mock.calls[1][0].map((option: DataSourceItem) => option.key).includes(DATA_SOURCE[2].key),
      ).toBeTruthy()
    ...
    it("supports multiple defaultValue (uncontrolled)", async () => {
      ...
      expect(
        onValueChange.mock.calls[0][0].map((option: DataSourceItem) => option.key).includes(DATA_SOURCE[1].key),
      ).toBeTruthy()
      expect(
        onValueChange.mock.calls[0][0].map((option: DataSourceItem) => option.key).includes(DATA_SOURCE[2].key),
      ).toBeTruthy()
    })
  ...
  describe("autofill", () => {
    it("should have a hidden select element for form autocomplete", async () => {
      ...
      options.forEach((option, index) => index > 0 && expect(option).toHaveTextContent(dataSource[index - 1].label))
