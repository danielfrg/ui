...
export interface NumberFieldVaryTriggerRenderProps
  extends NumberFieldVaryTriggerCommonProps, Button.ButtonRootRenderProps {
  "aria-controls": string | undefined
}

export type NumberFieldVaryTriggerProps<T extends ValidComponent | HTMLElement = HTMLElement> =
  NumberFieldVaryTriggerOptions & Partial<NumberFieldVaryTriggerCommonProps<ElementOf<T>>>
...
export function NumberFieldVaryTrigger<T extends ValidComponent = "button">(
  ...
  const [local, others] = splitProps(props as NumberFieldVaryTriggerProps, ["numberFieldVaryType", "onClick"])

  return (
    <Button.Root<Component<Omit<NumberFieldVaryTriggerRenderProps, keyof Button.ButtonRootRenderProps>>>
      tabIndex={-1}
      disabled={
        formControlContext.isDisabled() ||
        context.rawValue() === (local.numberFieldVaryType === "increment" ? context.maxValue() : context.minValue())
      }
