...
export interface ScrollAreaScrollbarCommonProps<T extends HTMLElement = HTMLElement> {
  ref: T | ((el: T) => void)
...
export interface ScrollAreaScrollbarRenderProps extends ScrollAreaScrollbarCommonProps {
  "data-orientation": "vertical" | "horizontal"
  "data-scrolling": string | undefined
  "data-hovering": string | undefined
}

export type ScrollAreaScrollbarProps<T extends ValidComponent | HTMLElement = HTMLElement> =
  ScrollAreaScrollbarOptions & Partial<ScrollAreaScrollbarCommonProps<ElementOf<T>>>
...
export function ScrollAreaScrollbar<T extends ValidComponent = "div">(
  ...
  const [local, others] = splitProps(props as ScrollAreaScrollbarProps, ["ref", "style", "orientation", "keepMounted"])

  const orientation = () => local.orientation ?? "vertical"
  const isVertical = () => orientation() === "vertical"
  const hasOverflow = () => (isVertical() ? context.hasOverflowY() : context.hasOverflowX())
  ...
  const content = () => (
    <Polymorphic<ScrollAreaScrollbarRenderProps>
      ...
      style={combineStyle(combineStyle(positionStyle(), thumbSizeStyle()), local.style)}
