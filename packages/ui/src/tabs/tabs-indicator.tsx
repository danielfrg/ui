 ...
 */
import { type JSX, type ValidComponent, createEffect, createSignal, on, onMount, splitProps } from "solid-js"
...
export type TabsIndicatorProps<T extends ValidComponent | HTMLElement = HTMLElement> = TabsIndicatorOptions &
  Partial<TabsIndicatorCommonProps<ElementOf<T>>>
...
export function TabsIndicator<T extends ValidComponent = "div">(props: PolymorphicProps<T, TabsIndicatorProps<T>>) {
  ...
  const computeStyle = () => {
    ...
    const offset =
      direction() === "rtl"
        ? -1 *
          ((selectedTab.offsetParent as HTMLElement)?.offsetWidth - selectedTab.offsetWidth - selectedTab.offsetLeft)
        : selectedTab.offsetLeft

    styleObj.transform =
      context.orientation() === "vertical" ? `translateY(${selectedTab.offsetTop}px)` : `translateX(${offset}px)`
