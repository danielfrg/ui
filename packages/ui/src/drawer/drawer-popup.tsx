...
export interface DrawerPopupCommonProps<T extends HTMLElement = HTMLElement>
	extends DialogContentCommonProps<T> {
	style: JSX.CSSProperties | string;
}

export interface DrawerPopupRenderProps
	extends DrawerPopupCommonProps,
...
> = DrawerPopupOptions & Partial<DrawerPopupCommonProps<ElementOf<T>>>;
 ...
 */
export function DrawerPopup<T extends ValidComponent = "div">(
	props: PolymorphicProps<T, DrawerPopupProps<T>>,
) {
	...
	const [local, others] = splitProps(props as DrawerPopupProps, [
		"ref",
		"style",
	]);
	...
	const onPointerUp = (e: PointerEvent) => {
		...
		const relevantDelta =
			side === "left" || side === "right" ? Math.abs(deltaX) : Math.abs(deltaY);
