...
import { callHandler, mergeDefaultProps, mergeRefs, visuallyHiddenStyles } from "../utils"
import { combineStyle } from "@solid-primitives/props"
import { type JSX, type ValidComponent, createEffect, createSignal, on, splitProps } from "solid-js"
...
export interface CheckboxInputCommonProps<T extends HTMLElement = HTMLInputElement> {
  id: string
  ref: T | ((el: T) => void)
...
export interface CheckboxInputRenderProps extends CheckboxInputCommonProps, FormControlDataSet, CheckboxDataSet {
  type: "checkbox"
  name: string
  value: string
  checked: boolean
...
export type CheckboxInputProps<T extends ValidComponent | HTMLElement = HTMLInputElement> = CheckboxInputOptions &
  Partial<CheckboxInputCommonProps<ElementOf<T>>>

/**
 * The native html input that is visually hidden in the checkbox.
 */
export function CheckboxInput<T extends ValidComponent = "input">(props: PolymorphicProps<T, CheckboxInputProps<T>>) {
  ...
  createEffect(
    on([() => ref, () => context.indeterminate(), () => context.checked()], ([ref, indeterminate]) => {
      if (ref) {
