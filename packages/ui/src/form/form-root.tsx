...
> = FormRootOptions & Partial<FormRootCommonProps<ElementOf<T>>>;

/**
 * A form wrapper that prevents default submission and provides an onSubmit handler.
 * Renders a native `<form>` element.
 */
export function FormRoot<T extends ValidComponent = "form">(
	props: PolymorphicProps<T, ParentProps<FormRootProps<T>>>,
) {
	const [local, others] = splitProps(
		props as ParentProps<FormRootProps>,
		["onSubmit"],
	);
	...
	return (
		<Polymorphic<FormRootRenderProps>
			as="form"
			onSubmit={onSubmit}
			{...others}
		/>
