...
export type SliderTrackProps<T extends ValidComponent | HTMLElement = HTMLElement> = SliderTrackOptions &
  Partial<SliderTrackCommonProps<ElementOf<T>>>
...
export function SliderTrack<T extends ValidComponent = "div">(props: PolymorphicProps<T, SliderTrackProps<T>>) {
  ...
  const [local, others] = splitProps(props as SliderTrackProps, ["onPointerDown", "onPointerMove", "onPointerUp"])
  ...
  function getValueFromPointer(pointerPosition: number) {
    ...
    const input: [number, number] = [0, context.state.orientation() === "vertical" ? rect.height : rect.width]
