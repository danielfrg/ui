...
export interface SliderValueLabelCommonProps<T extends HTMLElement = HTMLElement> {}

export interface SliderValueLabelRenderProps extends SliderValueLabelCommonProps, SliderDataSet {
  children: string
}

export type SliderValueLabelProps<T extends ValidComponent | HTMLElement = HTMLElement> = SliderValueLabelOptions &
  ...
  return (
    <Polymorphic<SliderValueLabelRenderProps> as="div" {...context.dataset()} {...(props as SliderValueLabelProps)}>
      {context.getValueLabel?.({
        ...
        context.state
          .values()
          .map((v) => context.state.getFormattedValue(v))
          .join(", ")}
    </Polymorphic>
  )
}
